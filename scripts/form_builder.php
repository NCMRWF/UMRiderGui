...
$SUBMIT_FONT_LINK = 'https://fonts.googleapis.com/css?family=Oswald:700';
  ...
  */
  ...
  if(trim($USER_MESSAGE) != '' || $USER_MESSAGE != NULL){
    ...
    $attr = $dom->createAttribute('src');
    $attr->value = '../styling/alert/alert.png';
...
//    $dom->appendChild($alert);
  ...
  file_put_contents("../outfiles/$usr/umtype.txt", $model_type);   // for the form_processor

  if(isset($_POST['Start_date']) && $_POST['Start_date'] != "" && $_POST['Start_date'] != NULL){
    file_put_contents("../outfiles/$usr/startdate.txt", $_POST['Start_date']);
    if(isset($_POST['End_date']) && $_POST['End_date'] != "" && $_POST['End_date'] != NULL)
      file_put_contents("../outfiles/$usr/enddate.txt", $_POST['End_date']);
    else file_put_contents("../outfiles/$usr/enddate.txt", 'None');
  }
  else{

    file_put_contents("../outfiles/$usr/startdate.txt", 'YYYYMMDD');
    file_put_contents("../outfiles/$usr/enddate.txt", 'None');
  }
  ...
  foreach($model->children() as $object){
    ...
    $forminputs = $dom->createElement('div');
    $attr = $dom->createAttribute('class');
    $attr->value = 'forminput';
    $forminputs->appendChild($attr);
    ...
    if($ob_type == 'text'){ //text type handler
      ...
      $input->appendChild($attr);
      $attr = $dom->createAttribute('style');
      $attr->value = "color:#404040; font-family: 'Open Sans Condensed', sans-serif; width: 170px; font-weight: bold; width: 500px";
      $input->appendChild($attr);
      if(isset($ob_script)){
    ...
    //==================================//
      ...
      $defaultval = trim($object->default);
    ...
    //==================================//
      ...
      foreach($values->children() as $val){
          ...
          // make all of them checked by default
          $attr = $dom->createAttribute('checked');
          $attr->value = 'checked';
          $input->appendChild($attr);
    ...
    //==================================//
     ...
     $form->appendChild($inputs);                      //append each and every input
      $form->appendChild($dom->createElement('br'));   // line breaker
    }
